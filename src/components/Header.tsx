...
export default function Header({ onAction, currentYear, onYearChange, selectedWeek, onWeekChange, tasksProjectsData, onProjectsFilterChange }: HeaderProps) {
  ...
  // Update slider position based on active item or hover
  const updateSliderPosition = (element: HTMLElement | null) => {
    if (!sliderRef.current || !element || !navContainerRef.current) return
  ...
  // Update slider on hover
  const handleItemHover = (to: string) => {
    const item = itemRefs.current[to]
    if (item) {
      setHoveredItem(to)
    ...
    <>
      ...
      >
        ...
        <div className="px-4 py-4 flex items-center justify-between border-b border-gray-200">
          ...
          {/* Navigation Menu - Center - with sliding background */}
          <nav 
            ref={navContainerRef}
            className="relative flex items-center gap-2 bg-gray-50 rounded-full p-1.5"
          ...
          >
            ...
            <div
              ref={sliderRef}
              className="absolute top-1.5 bottom-1.5 bg-black rounded-full opacity-0"
              style={{
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-2 px-5 py-3 rounded-full transition-colors duration-200 z-10 ${
                    isUnderSlider
