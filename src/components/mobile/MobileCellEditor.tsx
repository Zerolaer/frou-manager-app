...
import { hapticLightTap } from '@/platform/haptics'
...
const DESC_FIELD_CLASS = cn(TOUCH_SIZE, 'w-full min-w-0 !px-3 !py-0 text-base !leading-none rounded-xl border-gray-200')
...
type Props = {
  ...
  monthIndex: number
  year: number
  onApply: (sum: number, hasEntriesInCell: boolean, monthIndex: number) => void
  monthNames?: string[]
  ...
  return (
    <button
      ...
      className={cn(
        ICON_BTN,
        'h-full w-full',
        included ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-300 text-transparent'
      )}
      aria-label={label}
...
export default function MobileCellEditor({
  ...
  function updateItemLocal(id: string, patch: Partial<Entry>) {
    setItems((prev) => {
      ...
      onApply(sumIncluded(next), next.length > 0, monthIndex)
      return next
    })
  }
  ...
  const subcategorySection = isParentCategory && (canAddSubcategory || readOnly) && (
    <section className="space-y-2 rounded-xl border border-gray-200 bg-gray-50/80 p-3">
      {canAddSubcategory && onAddSubcategory ? (
        ...
        <p className="text-xs leading-relaxed text-gray-500">{t('finance.cannotAddSubcategory')}</p>
      )}
    </section>
  )
  ...
  return (
    <MobileModal
      ...
      footer={
        <div className={cn('flex items-center gap-2 py-3', MOBILE_MODAL_STACKED_PX)}>
          <div className={cn(TOUCH_SIZE, 'flex min-w-0 flex-1 flex-row items-center rounded-xl border border-gray-200 bg-gray-50 px-3')}>
          ...
          <ModalButton variant="secondary" onClick={onClose} className={cn(TOUCH_SIZE, 'shrink-0 px-5')}>
            {t('actions.close')}
          </ModalButton>
        </div>
      }
    >
      <div className={cn('space-y-3 pb-5', MOBILE_MODAL_STACKED_PX)}>
        {onMonthChange && (
          <div className="finance-mobile-month-chips">
            {monthNames.map((label, index) => (
              <button
                ...
                className={cn('finance-mobile-month-chip', index === monthIndex && 'is-active')}
              >
                {label}
              </button>
            ))}
        ...
        {loading ? (
          <div className="flex items-center justify-center py-10 text-sm text-gray-500">{t('common.loading')}</div>
        ) : readOnly ? (
          <>
            <section className="space-y-2 rounded-xl border border-gray-200 bg-gray-50/80 p-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500">{t('finance.aggregatedTotal')}</h3>
              <p className="text-2xl font-semibold tabular-nums text-gray-900">{fmt.format(aggregatedTotal)}</p>
              <p className="text-xs leading-relaxed text-gray-500">{t('finance.aggregatedTotalHint')}</p>
            </section>
            {subcategorySection}
          </>
        ) : (
          <>
            <section className="space-y-2 rounded-xl border border-gray-200 bg-gray-50/80 p-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500">{t('finance.addEntry')}</h3>
              ...
              <section className="space-y-2">
                {items.map((item, index) => (
                  <article
                    key={item.id}
                    className={cn(
                      'finance-mobile-fade-up space-y-2 rounded-xl border p-3 transition-all duration-200',
                    ...
                    <div className={ENTRY_ROW}>
                      <div className={ENTRY_CONTROL_ICON}>
                        <InclusionToggle
                          ...
                          onToggle={() => void toggleIncluded(item.id, !item.included)}
