...
export default function InvoiceFolderSidebar({ userId, activeId, onChange, collapsed = false, onToggleCollapse }: Props){
  ...
  async function onDropOver(id:string){
    ...
    setItems(withPos)
    await Promise.all(withPos.map(p=> supabase.from('invoice_folders').update({ position:p.position }).eq('id', p.id)))
  }
  ...
  return (
    <aside className="invoice-folders rounded-3xl bg-white" style={{ width: collapsed ? 72 : 280, border: '1px solid #E9F2F6', transition: 'width 0.3s cubic-bezier(0.4, 0, 0.2, 1)' }}>
      <div className="flex items-center mb-3" style={{ 
        ...
        <button 
          ...
          aria-label={t('invoice.addFolder')} 
          style={{ 
            position: 'absolute',
            right: '0',
            flexShrink: 0,
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('invoice.folderName')}</label>
            <ModalInput 
              value={createName} 
              onChange={(e)=>setCreateName(e.target.value)}
              placeholder={t('invoice.folderName')}
            />
          </div>
          <div>
      ...
      <UnifiedModal
        ...
        title={t('invoice.deleteFolder') || 'Delete Folder?'}
      ...
      >
        <div className="text-sm text-gray-600">{t('invoice.deleteFolderWarning') || 'Folder and related invoices will be deleted.'}</div>
      </UnifiedModal>
    </aside>
  )
}
